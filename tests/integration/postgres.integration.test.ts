// =====================================================
// JSDB - PostgreSQL Integration Tests
...
describe('PostgreSQL Integration', () => {
  ...
  describe('CRUD', () => {
    ...
    it('updateOne', async () => {
      const coll = client.collection(TEST_COLLECTION);
      await coll.insertOne({ name: 'Eve', age: 40, active: true });
    ...
    it('updateMany', async () => {
      ...
      const count = await coll.count({ active: true });
      expect(count).toBe(0);
    });
    ...
    it('deleteMany', async () => {
      const coll = client.collection(TEST_COLLECTION);
      await coll.insertMany([
        { name: 'Del1', active: true },
        { name: 'Del2', active: true },
        { name: 'Del3', active: false },
      ]);

      const result = await coll.deleteMany({ active: true });
  ...
  describe('Filters', () => {
    beforeEach(async () => {
      const coll = client.collection(TEST_COLLECTION);
      await coll.insertMany([
        { name: 'Alice', age: 30, score: 90.5, active: true },
        { name: 'Bob', age: 25, score: 85.0, active: true },
        { name: 'Charlie', age: 35, score: 95.0, active: false },
        { name: 'Diana', age: 28, score: 88.5, active: true },
        { name: 'Eve', age: 40, score: 92.0, active: false },
      ]);
    });
    ...
    it('$exists filter', async () => {
      const coll = client.collection(TEST_COLLECTION);
      const result = await coll.find({ score: { $exists: true } });
      expect(result.length).toBe(5);
    });

    it('null handling', async () => {
      const coll = client.collection(TEST_COLLECTION);
      await coll.insertOne({ name: 'NullTest', email: 'unique@test.com' });
      const result = await coll.find({ email: 'unique@test.com' });
      expect(result.length).toBe(1);
      expect(result[0].name).toBe('NullTest');
    });
    ...
    it('boolean values', async () => {
      const coll = client.collection(TEST_COLLECTION);
      const result = await coll.find({ active: true });
      expect(result.length).toBe(3);
    });
  });
  ...
  describe('Projection', () => {
    beforeEach(async () => {
      const coll = client.collection(TEST_COLLECTION);
      await coll.insertOne({ name: 'Alice', age: 30, email: 'alice@test.com', score: 95.5 });
    });

    it('field inclusion', async () => {
      const coll = client.collection(TEST_COLLECTION);
      const result = await coll.find({}, { projection: { name: 1, age: 1 } });
      expect(result.length).toBe(1);
      expect(result[0].name).toBe('Alice');
  ...
  describe('Raw Query', () => {
    ...
    it('executeRaw with params', async () => {
      ...
      const result = await client.raw(`SELECT * FROM "${TEST_COLLECTION}" WHERE name = $1`, ['RawTest']);
